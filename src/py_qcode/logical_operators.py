...
def squoct_log_ops(total_size, gauge=False):
    """
    ...
    edge_0 = sorted(range(0, total_size[
                    0], 6) + range(2, total_size[0], 6))
    edge_1 = sorted(range(0, total_size[
                    1], 6) + range(2, total_size[1], 6))
    shift_edge_0 = sorted(range(3, total_size[
                          0], 6) + range(5, total_size[0], 6))
    shift_edge_1 = sorted(range(3, total_size[
                          1], 6) + range(5, total_size[1], 6))

    # Tough to tell what is/is not a gauge operator. Alternate sets
    # defined below:
    if gauge:
        #logicals of the "sacrificed" qubit
